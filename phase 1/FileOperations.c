...
void Instructions_To_buffer()
{
    ...
    FILE *file = fopen("/Users/mangeshdhanmane/Desktop/CPU-Simulator/phase 1/input.txt", "r");

    int Linecounter = 1;

    while (Linecounter < current_line_Counter)
    { // Finds the line where the current job begins
...
void Data_To_Buffer(int memory_address)
{
    ...
    while (EOF)
    { // Gets the line that starts with $DATA
        fgets(buffer, sizeof(buffer), file);
