...
void Instructions_To_buffer()
{
    ...
    FILE *file = fopen("/Users/mangeshdhanmane/Desktop/CPU-Simulator/phase 2/input.txt", "r");
    int Linecounter = 1;

    while (Linecounter <= current_line_Counter)
    { // Finds the line where current job begins
    ...
    while (EOF)
    { // Gets the line that starts with $AMJ
        flush_Buffer();
        fgets(buffer, sizeof(buffer), file);
    ...
    while (1)
    { // Reads the instruction from the next line
        flush_Buffer();
        fgets(buffer, sizeof(buffer), file);
...
int Data_To_Buffer(int memory_address)
{
    ...
    while (fgets(buffer, sizeof(buffer), file) != NULL)
    { // Reads 40 bytes of data at a time
        flush_Buffer();
...
void Buffer_To_OutputFile(int block_address)
{
    ...
    for (int i = 0; i < 40; i++)
    {
        ...
        if (buffer[i] == '-')
        { // Printing the data in output file
            fputc(' ', file);
            continue;
        }
...
int check_next_job()
{
    ...
    if (!file)
    {
        ...
        return 0; // or suitable error code
    }
    flush_Buffer();
    int Linecounter = 1;
